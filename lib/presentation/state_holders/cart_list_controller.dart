...
class CartListController extends GetxController {
  ...
  void changeProductQuantity(int cartItem, int quantity) {
    _cartList.firstWhere((c) => c.id == cartItem).qty = quantity;
    update();
  }
  ...
  Future<bool> deleteCartItem(int cartItem) async {
    bool isSuccess = false;
    _inProgress = true;
    update();
    final NetworkResponse response = await NetworkCaller.getRequest(
      url: Urls.deleteCartItem(cartItem),
    );
    if (response.isSuccess) {
      _cartList.removeWhere((c) => c.id == cartItem);
      isSuccess = true;
    } else {
      _errorMessage = response.errorMessage;
    }
